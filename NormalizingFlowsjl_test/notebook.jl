...
# ╔═╡ 4419b13a-fb60-11ef-3e02-7ffc67ca5978
using NormalizingFlows, Lux, Enzyme, Plots, Random

# ╔═╡ 64cf3aa1-a149-47e5-b553-176edbefbfe0
function moon(n, center, radius, noise; rng=Random.Xoshiro(0))
	xcenter, ycenter = center
	radii = (radius .+ randn(rng, n)*noise)
	angle = randn(rng, n) * π/6
	x = sin.(angle).*radii .+ xcenter
	y = cos.(angle).*radii .+ ycenter
	return [x;;y]
end

# ╔═╡ 110521be-0451-47f7-be73-d5a08d2e99b3
begin
	data = moon(1000, (0.0,0.0), 1, 0.1)
	x = data[:,1]
	y = data[:,2]
	scatter(x, y, label="Moon 1", title="Two Moons")

	data1 = moon(1000, (0.7, -0.5), 1, 0.1)
	x = data1[:,1]
	y = -data1[:,2]
	scatter!(x, y, label="Moon 2")
	xlabel!("x")
	ylabel!("y")
end
...
manifest_format = "2.0"
project_hash = "c85c70d9d4f2801b1d467493e32044fb04b26f3c"
